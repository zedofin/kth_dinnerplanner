var View4 = function (container, model) {	
	...
	this.update = function(obj){
		...
		//container.find("#dishPeople").html(model.getNumberOfGuests());
		...
		model.getDish(model.getShowDish()[0], function(data){
			...
			for (i in ings){
				dishIngredientsHtml += "<tr><td>" + ings[i].amount / data.servings * model.getNumberOfGuests()  +" "+ ings[i].unit +"</td><td>"+ ings[i].name + "</td><td> SEK </td><td> " + ings[i].amount / data.servings * model.getNumberOfGuests() +"</td></tr>";
			}
